...
class tf_CNN:
    def __init__(self, inputShape, optimizer, learning_rate=0.1125,
        ...
        self.weights = {
            'wc1': tf.Variable(tf.random_normal([3, 2, 1, 6], stddev=0.2)),
            'wd1': tf.Variable(tf.random_normal([(self.L)*1*6, 5],
                                                stddev=np.sqrt(2./((self.L)*1*6+5)))),
            'out': tf.Variable(tf.random_normal([5, n_classes], stddev=0.1))
        }

        self.biases = {
            'bc1': tf.Variable(np.zeros(6, dtype=np.float32)),
