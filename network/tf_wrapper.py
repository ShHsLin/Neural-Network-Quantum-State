...
def batch_norm_new(bottom, phase, scope_name='bn'):
    ...
    return tf.layers.batch_normalization(inputs=bottom, axis=1 if data_format == 'channels_first' else 3,
              momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, center=True,
              scale=True, training=phase, fused=True, name=scope_name)
...
def pixel_block_sharir(x, in_channel, out_channel, block_type, name,
                       ...
                       layer_collection=None, registered=False,
                       residual_connection=False, batch_norm=False):
    '''
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if block_type == 'start':
            ...
            if batch_norm:
                vertical_branch = batch_norm(vertical_branch, phase=True)
            ...
            horizontal_branch = masked_conv_layer2d(hor_padded_x, filter_size, in_channel+out_channel//2, out_channel//2,
                                                    ...
                                                    registered=registered)
            if batch_norm:
                horizontal_branch = batch_norm(horizontal_branch, phase=True)
