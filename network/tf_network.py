...
class tf_network:
    def __init__(self, which_net, inputShape, optimizer, dim, sess=None,
                 ...
                 alpha_list=None, filter_size=None,
        ...
        if dim == 1:
            ...
            self.channels = int(inputShape[1])
            self.build_network = self.build_network_1d
            self.ordering = np.arange(self.L)
        elif dim == 2:
    ...
    def build_unaggregated_gradient(self):
        ...
        # i = tf.constant(0)
        def condition(i, _): return i < tf.shape(self.x)[0]
        ...
        def body(i, ta):
            ...
            if self.using_complex:
                ...
                ta = ta.write(i, tf.concat(
                    [tf.reshape(g, [-1]) for g in tf.gradients(single_log_amp, self.para_list, grad_ys=tf.complex(1., 0.))], axis=0))
            return (i+1, ta)
...
#                                                              list(zip(self.newgrads, F_list))})
    ...
    def build_MADE_hid1_phase_1d(self, x, activation):
        ...
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            ...
            fc2 = act(fc1)
            fc3 = tf_.masked_fc_layer(fc2, self.L * self.alpha, self.L * 2,
                                      'masked_fc3', self.ordering, 'B',
                                      layer_collection=self.layer_collection,
                                      registered=self.registered, dtype=self.TF_FLOAT)

            fc3 = tf.reshape(fc3, [-1, self.L, self.channels])

            out0_re = fc3[:, :, 0]
            out1_re = fc3[:, :, 1]

            fc1_phase = tf_.masked_fc_layer(x, self.L, self.L * self.alpha*2, 'masked_fc1_phase',
                                            self.ordering, 'A', layer_collection=self.layer_collection,
                                            registered=self.registered, dtype=self.TF_FLOAT)

            fc1_phase = tf.reshape(fc1_phase, [-1, self.L, self.alpha, 2])
            exp_fc1 = tf.exp(tf.complex(tf.zeros_like(fc1_phase), fc1_phase))
            log_sum_exp_fc1 = tf.imag(tf.log(tf.math.reduce_sum(exp_fc1, axis=-2)))

            out0_im = log_sum_exp_fc1[:, :, 0]
            out1_im = log_sum_exp_fc1[:, :, 1]
            ## stable normalize ##
    ...
    def build_MADE_1d(self, x, activation):
        ...
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            ...
            fc_end = tf_.masked_fc_layer(fc, self.L * self.alpha_list[-1], self.L * 4,
                                         'masked_fc%d' % (len(self.alpha_list) + 1), self.ordering, 'B',
                                         layer_collection=self.layer_collection,
                                         registered=self.registered, dtype=self.TF_FLOAT)

            fc_end = tf.reshape(fc_end, [-1, self.L, self.channels * 2])
    ...
    def build_MADE_dense_1d(self, x, activation):
        act = tf_.select_activation(activation)
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            x_input_head = x
            x = x_input_head[:, :, 0]
            ...
            concat_cache_list = [tf.reshape(fc, [-1, self.L, self.L, self.alpha_list[0]])]
            num_channel_sum = self.alpha_list[0]

            for idx in range(1, len(self.alpha_list)):
                fc = tf_.masked_fc_layer(fc, self.L * num_channel_sum, self.L * self.alpha_list[idx],
                                         'masked_fc%d' % (idx+1), self.ordering, 'B',
                                         layer_collection=self.layer_collection,
                                         registered=self.registered, dtype=self.TF_FLOAT)
                fc = act(fc)

                reshaped_fc = tf.reshape(fc, [-1, self.L, self.L, self.alpha_list[idx]])
                concat_cache_list.append(reshaped_fc)
                reshaped_fc = tf.concat([reshaped_fc, *concat_cache_list[:-1]], axis=-1)
                num_channel_sum = num_channel_sum + self.alpha_list[idx]
                fc = tf.reshape(reshaped_fc, [-1, self.L, self.L * num_channel_sum])

            fc_end = tf_.masked_fc_layer(fc, self.L * num_channel_sum, self.L * 4,
                                         'masked_fc%d' % (len(self.alpha_list) + 1), self.ordering, 'B',
                                         layer_collection=self.layer_collection,
                                         registered=self.registered, dtype=self.TF_FLOAT)
    ...
    def build_complex_MADE_1d(self, x, activation):
        act = tf_.select_activation(activation)
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            x_input_head = x
            x = x_input_head[:, :, 0]
            ...
            fc3 = tf.reshape(fc3, [-1, self.L, self.channels])
            fc3 = tf.concat([tf.real(fc3), tf.imag(fc3)], axis=-1)
    ...
    def build_pixelCNN_old_1d(self, x, activation):
        act = tf_.select_activation(activation)
        ...
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            ...
            for idx in range(1, len(self.alpha_list)):
                conv = tf.pad(conv,
                              [[0, 0], [filter_size // 2, filter_size // 2], [0, 0]], "CONSTANT")
                conv = tf_.masked_conv_layer1d(conv,
                                               filter_size,
                                               self.alpha_list[idx-1],
                                               self.alpha_list[idx],
                                               'B',
                                               'masked_conv%d' % (idx+1),
                                               padding='VALID',
            ...
            conv = tf.pad(conv,
                          [[0, 0], [filter_size // 2, filter_size // 2], [0, 0]], "CONSTANT")
            conv_end = tf_.masked_conv_layer1d(conv,
                                               filter_size,
                                               self.alpha_list[-1],
                                               4,
                                               'B',
    ...
    def build_pixelCNN_1d(self, x, activation):
        act = tf_.select_activation(activation)
        filter_size = self.filter_size
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            x_input_head = x
    ...
    def build_pixelCNN_dense_1d(self, x, activation):
        act = tf_.select_activation(activation)
        filter_size = self.filter_size
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            x_input_head = x
            ...
            for idx in range(1, len(self.alpha_list)):
                ...
                concat_cache_list.append(conv)
                conv = tf.concat([conv, *concat_cache_list[:-1]], axis=-1)
                num_channel_sum = num_channel_sum + self.alpha_list[idx]

                # #### Short cut
                # short_cut = tf_.conv_layer1d(conv1_tmp,
                #                              1,
                #                              self.alpha*4,
                #                              self.alpha*3,
                #                              "short_cut_1",
    ...
    def build_ARTN_1d(self, x, activation):
        ...
        with tf.variable_scope("network", reuse=tf.AUTO_REUSE):
            ...
            input_mask = tf.cast(x_input_head, self.TF_COMPLEX)
            # now a complex tensor of shape [batch_size, L , 2]
            input_mask_extend = tf.tensordot(tf.cast(x_input_head, self.TF_COMPLEX),
                                             tf.ones([2, 2], dtype=self.TF_COMPLEX), axes=0)
...
#             return tf.real(out), None, log_cond_amp, prob
    ...
    def build_network_1d(self, which_net, x, activation, num_blocks):
        ...
        elif which_net == "MADE_dense":
            return self.build_MADE_dense_1d(x, activation)
        elif which_net == "MADE_hid1_phase":
            return self.build_MADE_hid1_phase_1d(x, activation)
        elif which_net == "complex_MADE":
            return self.build_complex_MADE_1d(x, activation)
        elif which_net == "pixelCNN_old":
            return self.build_pixelCNN_old_1d(x, activation)
        elif which_net == "pixelCNN":
            return self.build_pixelCNN_1d(x, activation)
        elif which_net == "pixelCNN_dense":
