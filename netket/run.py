...
def run_simulation(L, T, alpha, seed, str_lr='1e-1'):
    method = 'sr'
    np.random.seed(seed)
    ...
    if method == 'Gd':
        ...
        for i in range(1, 1+n_iter):
            ...
            if i > 100 and overlaps[-1] < 0.1:
                break

            elif i > 5000 and ( np.mean(np.array(overlaps)[-50:]) > 1 - 1e-4 or np.isclose(np.mean(np.array(overlaps)[-50:]),
                                                                                          np.mean(np.array(overlaps)[-100:-50]), atol=3e-7, rtol=3e-6)):
                break
